                ...
                <nav>
                    ...
                    <ol class="flex flex-wrap pt-1 mr-12 bg-transparent rounded-lg sm:mr-16">
                        ...
                        <li class="text-sm pl-2 capitalize leading-normal text-white before:float-left before:pr-2 before:text-white before:content-['/']"
                            aria-current="page">Karyawan</li>
                    </ol>
                    <h6 class="mb-0 font-bold text-white capitalize">Karyawan</h6>
                </nav>
            </div>
        </nav>
        <!-- end navbar -->
                                        ...
                                        @foreach ($employees as $employee)
                                            <tr>
                                                ...
                                                <td
                                                    class="px-6 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                                                    <h6 class="mb-0 text-sm leading-normal dark:text-white">
                                                        {{ $employee->position->name }}</h6>
                                                </td>
                                                <td
                                                    class="px-6 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                                                    <h6 class="mb-0 text-sm leading-normal dark:text-white">
                                                        {{ $employee->contract->contract_type }}</h6>
                                                </td>
                                                <td
                                                    class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                                                    <a href="javascript:;"
                                                        onclick="openEditSwal('{{ $employee->id }}', '{{ $employee->name }}', '{{ $employee->position_id }}', '{{ $employee->contract_id }}')"
...
<script>
    ...
    function openCreateSwal() {
        ...
        let contractOptions = contracts.map(ct =>
            `<option value="${ct.id}">${ct.contract_type}</option>`
        ).join('');

        Swal.fire({
            title: 'Tambah Karyawan',
            html: `
            <input id="swal-name" class="swal2-input" placeholder="Nama Karyawan">
            </br>
            <label>Jabatan</label>
            <select id="swal-position" class="swal2-input">${positionOptions}</select>
            </br>
            <label>Jenis Kontrak</label>
            <select id="swal-contract" class="swal2-input">${contractOptions}</select>
        `,
            ...
            preConfirm: () => {
                ...
                const contract_id = document.getElementById('swal-contract').value;

                if (!name || !position_id || !contract_id) {
                    Swal.showValidationMessage('Semua field wajib diisi');
                    return false;
                }

                return fetch(`{{ route('employees.store') }}`, {
                    method: 'POST',
                    headers: {
    ...
    function openEditSwal(id, currentName, selectedPositionId, selectedContractId) {
        let positionOptions = positions.map(pos =>
            `<option value="${pos.id}" ${pos.id == selectedPositionId ? 'selected' : ''}>${pos.name}</option>`
        ).join('');

        let contractOptions = contracts.map(ct =>
            `<option value="${ct.id}" ${ct.id == selectedContractId ? 'selected' : ''}>${ct.contract_type}</option>`
        ).join('');

        Swal.fire({
            title: 'Edit Karyawan',
            html: `
            <input id="swal-name" class="swal2-input" placeholder="Nama Karyawan" value="${currentName}">
            </br>
            <label>Jabatan</label>
            <select id="swal-position" class="swal2-input">${positionOptions}</select>
            </br>
